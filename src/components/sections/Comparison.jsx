 ...
 * renders a dash plus the words, so the table never leaves meaning to a glyph
 * or a colour alone.
 */
function Value({ value, tone }) {
  ...
  return (
    <span className={cx('flex items-start gap-s3', tone === 'super' ? 'text-text-primary' : 'text-text-muted')}>
      <span
        aria-hidden="true"
        className={cx(
          'mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full',
          tone === 'super' ? 'bg-action-primary text-white' : 'bg-surface-strong text-text-muted',
        )}
      >
...
/** One column's caption inside a stacked mobile row. */
function MobileCaption({ children, tone }) {
  return (
    <span
      className={cx(
        'mb-s2 block text-xs font-semibold uppercase tracking-[0.12em] sm:hidden',
        tone === 'super' ? 'text-action-primary-active' : 'text-text-muted',
      )}
    >
      {children}
    </span>
    ...
    <Section id="compare" aria-labelledby="compare-title" className="overflow-hidden bg-surface-base">
      ...
      <Reveal delay={100} className="mx-auto max-w-5xl">
        {/*
          ...
          collapsed borders ignore `border-radius`, so the highlighted column's
          2px frame ran into the container's rounded corner and was sliced off
          by `overflow-hidden`. Separated borders let the frame carry its own
          27px radius and curve *inside* the container's 28px one.
        */}
              ...
              {rows.map((row, i) => (
                ...
                >
                  <th
                    scope="row"
                    className={cx(
                      'px-s6 py-s7 text-md font-semibold text-text-primary',
                  ...
                  >
                    <MobileCaption>{columns.premium.name}</MobileCaption>
                    <Value value={row.premium} tone="premium" />
                  </td>

                  <td
                    className={cx(
                      'px-s6 py-s7 text-md sm:border-x-2 sm:border-action-primary sm:bg-dream-100/40',
                  ...
                  >
                    <MobileCaption tone="super">{columns.superPremium.name}</MobileCaption>
                    <Value value={row.superPremium} tone="super" />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="mt-12 flex justify-center">
          <Button
            ...
            icon="arrowRight"
            wrap
            className="max-w-[19rem] text-center sm:max-w-none"
          >
